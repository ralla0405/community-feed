...
import store.kirinit.communityfeed.post.repository.entity.comment.QCommentEntity;
import store.kirinit.communityfeed.post.repository.entity.like.LikeTarget;
...
public class CommentQueryRepositoryImpl {
    ...
    public List<GetContentResponseDto> getCommentList(Long postId, Long userId, Long lastContentId) {
        return queryFactory
            ...
            .from(commentEntity)
            .join(userEntity).on(commentEntity.writer.id.eq(userEntity.id))
